...
struct GridIndexHash {
  size_t operator()(const GridIndex& idx) const {
    return std::hash<int>()(idx.first) ^ (std::hash<int>()(idx.second) << 1); // Optimized hash
  }
};
struct pair_hash {
...
std::mutex map_mutex_; // Mutex to protect global_occupancy_map_
...
public:
  PointCloudToOccupancyGridNode() : Node("pointcloud_to_occupancy_grid_node") {
    ...
    // Timer for publishing local map
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(100), // Increased frequency for smoother updates
...
void update_global_map(const sensor_msgs::msg::PointCloud2::SharedPtr msg, int8_t occupancy_value) {
    ...
    if (occupancy_value == 100) {
        obstacle_map_.swap(local_map); // 障碍物 map 用 swap 替换
    } else if (occupancy_value == 0) {
        ground_map_.swap(local_map);   // 地面 map 用 swap 替换
    }

    // 可选：限制 map 范围，只保留机器人周围 N*resolution 范围
    const int radius_cells = 200; // 半径 20m
...
//   }
  ...
  void obstacle_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    // {
    //   std::lock_guard<std::mutex> lock(map_mutex_);
    //   global_occupancy_map_.clear();  // 清空，避免持续增长
    // }
    auto end1 = std::chrono::high_resolution_clock::now();    // 结束时间
  ...
  void ground_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    auto end2 = std::chrono::high_resolution_clock::now();    // 结束时间
    // {
    //   std::lock_guard<std::mutex> lock(map_mutex_);
    //   global_occupancy_map_.clear();  // 清空，避免持续增长
    // }
